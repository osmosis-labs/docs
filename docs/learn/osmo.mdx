...
Each day's newly minted OSMO is split between several uses by a governance-controlled set of proportions. As of writing, the split is weighted toward staking rewards, developer rewards, and the community pool (which governance then allocates, including to liquidity incentives). Because these proportions and the reduction schedule are governance parameters, they can change by proposal. For the live values and the mechanism, see the [Mint module page](/build/chain/mint) under Build.
