...
Creating a new orderbook market is permissionless: anyone can open a market for a new pair. The orderbook is implemented as a smart contract; for the developer-facing detail (discovering markets, placing and cancelling orders, querying state, and routing), see the [Orderbook module page](/build/chain/pool-manager/cosmwasmpool/orderbook) under Build.
