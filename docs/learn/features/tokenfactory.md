...
It also handles the token's initial liquidity. Rather than asking the creator to seed a pool upfront, a new token trades against a bonding curve: buyers trade into a virtual pool where the price rises along the curve as more is bought. Once the curve has accumulated enough OSMO to reach its graduation threshold, the token "graduates" and its accumulated liquidity becomes a real tradeable pool on Osmosis. This bootstraps liquidity from trading activity itself, so a community can launch a token without putting up initial capital.
