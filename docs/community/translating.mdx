...
```
3. Copy the English language files to the new `{lang}` files (the `.json` localization file and the `.js` dayjs locale file). For example from inside the osmosis-frontend repo:  
