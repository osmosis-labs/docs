...
- **SQS**: the [`/pools`](/integrate/endpoints/sqs) endpoint returns orderbook pools alongside CFMM and concentrated liquidity pools. This is the recommended path for any frontend or bot that needs to know the active orderbook markets.
...
For the full pool-creation walkthrough (creation fee, denom-casing caveats, the `osmosisd` version requirement), see the [Pool Setup guide](/integrate/list-asset/pool-setup).
