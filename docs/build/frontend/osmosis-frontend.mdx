...
Translations live as JSON files under `packages/web/localizations` in the frontend repo. To add or edit a language, fork the repo, edit the relevant `{lang}.json` file, and open a PR. See [Frontend translations](../../community/translating) for the full step-by-step workflow.
