...
```
![](@site/docs/assets/local-installer.png)
...
```
cd artifacts
osmosisd tx wasm store cw_tpl_osmosis.wasm  --from <unsafe-test-key-name> --chain-id=<chain-id> --gas-prices 0.05uosmo --gas auto --gas-adjustment 1.3 -y
```
...
```
TX=$(osmosisd tx wasm store cw_tpl_osmosis.wasm  --from <unsafe-test-key-name> --chain-id=<chain-id> --gas-prices 0.05uosmo --gas auto --gas-adjustment 1.3 --output json -y | jq -r '.txhash')
...
osmosisd tx wasm instantiate $CODE_ID $INITIAL_STATE --amount 50000uosmo  --label "Counter Contract" --from <unsafe-test-key-name> --chain-id <chain-id> --gas-prices 0.05uosmo --gas auto --gas-adjustment 1.3 -y --no-admin
```

Example
```
INITIAL_STATE='{"count":100}'
osmosisd tx wasm instantiate $CODE_ID $INITIAL_STATE --amount 50000uosmo  --label "Counter Contract" --from c1 --chain-id localosmosis --gas-prices 0.05uosmo --gas auto --gas-adjustment 1.3 -y --no-admin
```
