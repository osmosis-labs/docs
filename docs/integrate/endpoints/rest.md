...
The list of all available REST endpoints is available as a Swagger specification file, it can be viewed at `localhost:1317/swagger`. Make sure that the `api.swagger` field is set to true in your `app.toml` file. You can also browse and try the LCD endpoints interactively in the [API Reference](/api).
