...
Note: Creating a pool requires a pool creation fee, set to 20 USDC by [Proposal 767](https://www.mintscan.io/osmosis/proposals/767) (query the poolmanager params for the live value).
...
The smoothest flow is to add an initial liquidity position immediately after creating the pool, without closing the modal. If the pool creation flow is interrupted before initial liquidity has been added (for example, by closing the modal or navigating away), you can still add liquidity afterward: locate the new pool from its asset page in the Osmosis Zone app and add a position there. Initial liquidity can also be added via the osmosisd CLI (see [Join Pool](#join-pool) for steps).
...
```bash
osmosisd tx concentratedliquidity create-pool ibc/... uosmo 100 0.01 --from val --chain-id osmosis-1 --gas auto --gas-adjustment 1.3 --gas-prices 0.05uosmo
```
...
```bash
osmosisd tx concentratedliquidity create-position 1 "[-108000000]" 342000000 10000ibc/...,10000uosmo 100 100 --from val --chain-id osmosis-1 --gas auto --gas-adjustment 1.3 --gas-prices 0.05uosmo
```
...
- `token-0-min-amount` and `token-1-min-amount` refer to the minimum amount of token-0 and token-1, respectively, that must be added to the position. These values must also be less than the corresponding amounts of tokensProvided because the transaction cannot add more tokens than were approved for addition.
  - To find out which is token-0, and which is token-1, query: `osmosisd query poolmanager pool <pool-id>`. If you created the pool in the previous step (when adding liquidity for the first time to the pool), the pool ID will be in the `create-pool` output.
...
Multiple alloyed-transmuter code IDs are whitelisted at once, and the most recent whitelisted ID should be used when creating a new pool. The code IDs current at the time of writing, for convenience, are:

- osmosis-1 (mainnet): 996
- osmo-test-5 (testnet): 11749

Confirm the latest whitelisted ID before creating a pool rather than relying on these values (see [Finding the current code ID](/integrate/features/transmuter#finding-the-current-code-id)).
...
```bash
osmosisd tx cosmwasmpool create-pool 1 '{"pool_assets_denom":["uion","uosmo"]}' --from lo-test1 --keyring-backend test --chain-id localosmosis --fees 875uosmo 
```
...
Sample pool JSON file contents for balancer (a 40/40/20 ATOM/OSMO/AKT pool; non-native assets must use their full Osmosis `ibc/HASH` denom, never the origin-chain denom like `uatom`):

```json
{
  "weights": "4ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2,4uosmo,2ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4",
  "initial-deposit": "100ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2,5uosmo,20ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4",
  "swap-fee": "0.001",
  "exit-fee": "0",
