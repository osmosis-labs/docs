...
The ```lockup``` module provides an interface for users to lock tokens (also known as bonding) in order to receive incentives.
...
```bash
osmosisd tx lockup lock-tokens 25527546134174465309gamm/pool/13 --duration="168h" --from WALLET_NAME --chain-id osmo-test-3
```
