...
The ```incentives``` module provides users the functionality to create gauges, which distributes reward tokens to the qualified lockups (from the ```lockup``` module). 

[comment]: <> (Each lockup has designated lockup duration that indicates how much time that the user has to wait until the token release after they request to unlock the tokens.)
